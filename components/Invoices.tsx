...
const Invoices: React.FC = () => {
  const { invoices, inventory, services, workers, settings, addInvoice, updateInvoice, updateInvoiceStatus } = useApp();
  ...
  const toggleStatus = (inv: Invoice) => {
    const newStatus = inv.status === InvoiceStatus.PAID ? InvoiceStatus.UNPAID : InvoiceStatus.PAID;
    updateInvoiceStatus(inv.id, newStatus);
  };

  const PrintableDocument = ({ doc }: { doc: Invoice }) => {
    const assignedWorker = workers.find(w => w.id === doc.assignedWorkerId);
    return (
      <div className="printable-area bg-white text-black p-10 min-h-screen flex flex-col font-sans">
        <div className="flex justify-between border-b-4 border-slate-900 pb-8 mb-8">
          <div>
            <h1 className="text-4xl font-black text-slate-900 tracking-tighter uppercase">{settings.shopName}</h1>
            <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mt-1">{settings.tagline}</p>
            <div className="mt-6 text-[11px] font-bold text-slate-600 space-y-0.5">
              <p>{settings.address}</p>
              <p>Cell: {settings.phone} | NTN: {settings.ntn}</p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-2 gap-4 mb-10">
          ...
          <div className="bg-slate-50 p-6 rounded-2xl">
            <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">Assigned Technician</p>
            <p className="text-lg font-black text-slate-700 uppercase">{assignedWorker ? assignedWorker.name : 'Workshop General'}</p>
          </div>
        </div>
        ...
        <div className="mt-12 pt-8 border-t-2 border-slate-900 flex justify-between items-start">
          <div className="text-[8px] text-slate-400 max-w-sm leading-relaxed whitespace-pre-wrap">
      ...
      <div className="space-y-4 no-print">
        ...
        {filteredDocs.length === 0 ? (
          ...
          filteredDocs.map(inv => {
            const worker = workers.find(w => w.id === inv.assignedWorkerId);
            return (
              <div key={inv.id} className="bg-white dark:bg-slate-800 p-8 rounded-[2.5rem] border border-slate-50 dark:border-slate-700 flex flex-col lg:flex-row justify-between items-center gap-6 group hover:shadow-xl transition-all">
                      ...
                      {worker && (
                        <>
                          ...
                          <span className="flex items-center gap-1 text-[10px] font-black text-emerald-500 uppercase tracking-widest">
                            <span className="material-icons-round text-[12px]">engineering</span>
                            {worker.name}
                          </span>
                        </>
                      )}
                ...
                <div className="flex items-center justify-between w-full lg:w-auto gap-10 border-t lg:border-t-0 pt-4 lg:pt-0 border-slate-50 dark:border-slate-700">
                  <div className="text-right">
                    <p className="text-2xl font-black text-slate-800 dark:text-white tracking-tighter">PKR {inv.amount.toLocaleString()}</p>
                    <button 
                      onClick={() => toggleStatus(inv)}
                      className={`text-[9px] font-black uppercase tracking-widest mt-1 px-3 py-1 rounded-full border transition-all ${
          ...
          <div className="bg-white dark:bg-slate-800 w-full max-w-4xl sm:rounded-[3rem] rounded-t-[3rem] p-10 max-h-[95vh] overflow-y-auto shadow-2xl animate-in slide-in-from-bottom duration-300">
            ...
            <form onSubmit={handleAddInvoice} className="space-y-8">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-slate-400 uppercase ml-2 tracking-widest">Job Date</label>
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-slate-400 uppercase ml-2 tracking-widest">Assign Technician</label>
                  <select 
                    className="w-full p-5 bg-slate-50 dark:bg-slate-700 rounded-2xl border-none font-bold dark:text-white focus:ring-2 focus:ring-emerald-500 transition-all uppercase text-xs" 
                    value={assignedWorkerId} 
                    onChange={e=>setAssignedWorkerId(e.target.value)}
                  >
                    <option value="">Workshop Pool (None)</option>
                    {workers.map(w => (
